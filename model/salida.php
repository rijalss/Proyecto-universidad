	...
	function registrar($idproducto, $cantidad, $precio, $idempleado)
	{
		...
		try {
			...
			for ($i = 0; $i < count($idproducto); $i++) {
				...
				// Actualizar cantidadMostrador sumando la misma cantidad que se resta de cantidadExistencia
				$co->query("UPDATE existencia SET cantidadMostrador = cantidadMostrador + {$cantidad[$i]} WHERE clExistencia = $idProd");
			}
	...
	function registrarMostrador($idproducto, $cantidad, $precio, $idempleado)
	{
		...
		try {
			...
			for ($i = 0;
				...
				$co->query("UPDATE existencia SET cantidadMostrador = $Total WHERE clExistencia = $idProd");
			}
